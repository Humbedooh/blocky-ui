...
renderRules = (json, edit) ->
    ...
    if edit
        alert("Rules updated!")
    app(main, mk('input', { type:"button", onclick: "addRule();", value: "Add a new rule"}))
    app(main, mk('br'))
...
addRule = (rule) ->
    ...
    app(fd, btn)
    if rule.id
        btn = mk('input', {type: 'button', style: "margin-left: 50px;", class: 'btn btn-danger', value: "Delete rule", onclick: 'deleteRule("'+(rule.id||"")+'");'})
        app(fd, btn)
    app(form, fd)
